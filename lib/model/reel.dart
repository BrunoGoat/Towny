...
class Reel {
  Reel._(this.steps, this.seconds, this._mark);
  ...
  static Reel? of(List<Habit> habits, {double seconds = 62.0}) {
    ...
    // Primero lo que pesa cada cosa, y después en qué segundo cae. En dos
    // vueltas porque el total hace falta para repartir, y el total no se sabe
    // hasta haberlo recorrido entero.
    final weight = <double>[];
    var sum = 0.0;
    ...
    return Reel._(all, seconds, mark);
  }
  ...
  ReelMoment at(double t, int habits) {
    final counts = List<int>.filled(habits, 0);
    if (steps.isEmpty) {
