/// El dibujo de la marca de un hábito, sin pantalla de por medio.
///
/// Cada glifo está trazado a mano dentro de una caja de cien por cien, con el
/// mismo vocabulario con el que está hecho el pueblo: tiradas rectas, masas
/// escuadradas y unas pocas curvas honradas. De eso se trata — la marca que va
/// sobre un pueblo tiene que parecer que salió del mismo sitio que el pueblo, y
/// ninguna tipografía de ningún teléfono iba a conseguir eso.
///
/// Está en `engine/` y no en `ui/` porque **el renderizador la necesita**: la
/// marca va tallada en el cartel de cada pueblo, dentro de la escena. Mientras
/// vivía entre las pantallas, el motor tenía que importar hacia arriba para
/// dibujar un cartel, que es la única flecha del proyecto que no podía existir:
/// `engine/` no sabe nada de `model/` a propósito, y saber de `ui/` era peor.
///
/// Aquí no hay ni un widget. El de verdad —el que se pone en una fila de una
/// hoja— es [HabitSigil], y vive donde le toca, en `ui/habit_sigil.dart`.
library;
